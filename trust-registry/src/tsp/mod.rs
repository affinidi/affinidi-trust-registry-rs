...
/// shared dispatcher, and seals the response back to the sender.
///
/// `packed` is the CESR/qb64 stored string carried by
/// `InboundFrame::Tsp` — so we unpack with `atm.tsp().unpack` (which
/// base64url-decodes first), **not** `unpack_bytes` (that is for the raw
/// `connect_websocket` path, which yields already-decoded qb2). The mediator
/// permits only one websocket per DID, so the registry must never open a second
/// TSP socket — TSP frames arrive multiplexed on the DIDComm pickup stream.
pub async fn process_tsp_frame(
    ...
    let reply = handle_inbound(dispatcher, admin_dids, my_vid, &sender_did, doc).await;
