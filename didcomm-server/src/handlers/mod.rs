...
impl<R: TrustRecordRepository + 'static> MessageHandler for BaseHandler<R> {
    async fn handle(
        ...
        let ph = self
            ...
            .iter()
            .find(|ph| ph
                .get_supported_inboud_message_types()
                .contains(message_type)
            );
        if let Some(protocol_handler) = ph {
            info!("[profile = {}, type = {}, from = {}] new message", &profile.inner.alias, message_type, from);
            protocol_handler.handle(atm, profile, message, meta).await?;
        } else {
            // send problem report
            warn!("No handler found. Send problem report or ignore. message_type = {}, from = {}", &message.type_, from);
        }
        Ok(())
    }
}
